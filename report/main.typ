...
learning concerned with decision-making. 
It is used in scenarios where an intelligent agent 
takes actions in an environment in order to maximize a reward 
(e.g. a chess agent learning to maximize the chance of wining). 
...
Over the course of training the agent adapts its policy in order to maximize rewards (see @fig-rl).
...
a convolutional neural network (see @fig-conv-net) @cnn, 
and a residual network (see @fig-res-net) @resnet.
...
/ Medium: The reward set of medium sparsity. 
  They were modeled after chess principles that align with winning on all levels of play: 
  material advantage, controlling the center, winning, king safety
/ Dense: The dense reward-set was modeled after chess principles 
  which align with winning on lower levels of play. 
  On higher levels though, the rules are not the best option. 
  The rewards included:
  control of the outer center, control of the center, early castling, pawn promotion,
  blunder prevention, material advantage, king safety, giving checks, and winning.
